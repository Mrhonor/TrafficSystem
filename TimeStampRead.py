...
def shibie(A, train, test, trainingDigits):
    label_list = []
...
def result(video_pictures, trainDir, testDir):   ##计算时间戳与摄像头编号并返回
    train = (listdir(trainDir))
    test = (listdir(testDir))
    ...
    if(video_pictures[50]==1280):       ##判断视频宽度是否为1280
        for i in range(0,25):               ##循环识别前25帧
            ...
            x = shibie(A, train, test, trainDir)
    ...
    float(Start_time)
    float(End_time)
    results.append(Start_time/1000)  ##最终结果增添到返回list
    results.append(End_time/1000)
    results.append(CameraID)
    return results
